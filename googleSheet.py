    ...
    def prepareUpdateScheduledHours(self, worksheet:list, scraped_scheduled_data: dict[int, dict[int, dict[str, str]]]):
        ...
        for week_index in scraped_scheduled_data:
            for day_index in scraped_scheduled_data[week_index]:
                ...
                # First row in entireblock to edit (to remove empty rows at start of sheet)
                if not first_row:
                    first_row = row_num + 1
                ...
                if date_exists and row_num:
                    ...
                    sheet_values_new_rows.append(self.rowValues(0, date_str, week_day, shift_start=start_time, shift_end=end_time, holiday=holiday, day_off=day_off))

        index_num = 0
        for row_num in range(len(sheet_values_new_rows))[::-1]:
            index_num += 1

            sheet_values_new_rows[row_num][4] = SpreadSheetEquations.hoursScheduled(index_num + 1)
            sheet_values_new_rows[row_num][7] = SpreadSheetEquations.hoursWorked(index_num + 1)
            sheet_values_new_rows[row_num][8] = SpreadSheetEquations.nightHoursWorked(index_num + 1)
            sheet_values_new_rows[row_num][11] = SpreadSheetEquations.nightHoursScheduled(index_num + 1)
